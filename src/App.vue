...
const spotImage = (spot) => spot.image || `https://picsum.photos/seed/${encodeURIComponent(spot.name)}/900/520`
const mapLink = (address) => `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`
const onImageError = (event, name) => {
  event.target.src = `https://placehold.co/900x520/e2e8f0/334155?text=${encodeURIComponent(name)}`
}
</script>
        ...
        <Card class="p-4 md:col-span-2">
          ...
          <div class="space-y-3">
            <div v-for="spot in currentDay.spots" :key="spot.name" class="rounded-xl border border-blue-100 bg-white p-3">
              <img :src="spotImage(spot)" :alt="spot.name" @error="onImageError($event, spot.name)" class="mb-2 h-44 w-full rounded-lg object-cover" />
